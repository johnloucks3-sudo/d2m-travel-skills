...
class TestCollectRows:
    def test_collects_local_rows(self):
        rows = sheet_sync.collect_rows(include_gmail=False)
...
class TestDryRunCli:
    def test_dry_run_writes_json_with_all_rows_linked(self, tmp_path):
        ...
        rc = sheet_sync.main(["--dry-run", "--no-gmail", "--out", str(out)])
        assert rc == 0
    ...
    def test_dry_run_touches_no_google_config(self, tmp_path):
        ...
        sheet_sync.main(["--dry-run", "--no-gmail", "--out", str(tmp_path / "r.json")])
