#!/usr/bin/env python3
"""
rocket_gate.py — CMSgt Steve "Rocket" Sterling's back-of-task check battery.
Deterministic gates (Rocket never guesses a pass). Run before a portal reaches
the Commander or a client. Emits a visible CHIEF SILVER verdict.

Usage: python3 scripts/rocket_gate.py <portal_client_dir> [--others name1,name2]
...
def board_hygiene():
    """Rocket rides herd on the boards — surfaces anything left unresolved (the
...
def internal_ops_check():
    """Rocket on internal processes (2026-07-04): same board-hygiene/no-orphan
