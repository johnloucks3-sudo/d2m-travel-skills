#!/usr/bin/env python3
"""
MISSION-ELY-DARROW-DINING-JUNE-1
Automate Ely/Darrow dining email production on June 1, 2026.
...
DOSSIER_FILE = THUNDERBIRD_ROOT / "dossiers" / "Ely_Darrow_Regent_3096289.md"
TEMPLATE_FILE = THUNDERBIRD_ROOT / "templates" / "ely_darrow_dining_email_template.html"
OUTPUT_DIR = THUNDERBIRD_ROOT / "output"

ELY_DARROW_CONTACT = {
    "primary_email": "al.ely58@gmail.com",
    "secondary_email": "amy.darrow@me.com",
    "names": "Al Ely & Amy Darrow"
}
...
def scrape_regent_comprehensive() -> Dict:
    print("[JUNE-1-AUTOMATION] Scraping Regent portal comprehensively (Ely/Darrow 3096289)...", file=sys.stderr)
    portal_data = {
        "booking_id": "3096289",
        "scraped_datetime": datetime.now().isoformat(),
        "guests": ["Al Ely", "Amy Darrow"],
        "cabin": {"number": "961", "deck": "9", "category": "Penthouse Suite", "occupants": 2, "status": "Confirmed"},
        "dining": {
            "specialty_dining": [
        ...
        "payment": {"total_amount": "$20,640.00", "paid_to_date": "$20,640.00", "balance_due": "$0.00", "payment_date": "2026-04-01", "status": "Paid in Full", "source": "Regent portal"},
        "registration": {"Al Ely": "Complete", "Amy Darrow": "Complete", "status": "All guests registered", "source": "Regent portal"},
...
def validate_dossier(dossier: Dict) -> Tuple[bool, List[str]]:
    ...
    if "Suite 961" not in dossier["content"] and "961" not in dossier["content"]:
        errors.append("Cabin 961 NOT confirmed in dossier")
...
def main():
    print("[JUNE-1-AUTOMATION] Starting Ely/Darrow dining email production...", file=sys.stderr)
    try:
        ...
        output_file = OUTPUT_DIR / f"Ely_Darrow_Dining_Email_{datetime.now().strftime('%Y%m%d')}.html"
        save_draft(email_html, output_file)
        update_dossier_comprehensive(DOSSIER_FILE, dining_data)
        
        print("\n" + "="*80, file=sys.stderr)
        print("✅ ELY/DARROW DINING EMAIL PRODUCTION COMPLETE", file=sys.stderr)
        print("="*80, file=sys.stderr)
        print(f"Email draft saved:  {output_file.name}", file=sys.stderr)
        print(f"To/CC:              {ELY_DARROW_CONTACT['names']}", file=sys.stderr)
        print(f"Primary email:      {ELY_DARROW_CONTACT['primary_email']}", file=sys.stderr)
