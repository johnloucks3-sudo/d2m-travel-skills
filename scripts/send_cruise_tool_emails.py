#!/usr/bin/env python3
"""Send cruise tool feedback emails — 4 recipients, CC john + d2m."""
      ...
      <tr><td style="padding:0 32px 24px;font-size:15px;color:#e8eaf0;line-height:1.75;">
        ...
        <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 20px;">
          <tr><td style="background:rgba(240,192,64,0.09);border-left:3px solid #f0c040;border-radius:4px;padding:14px 18px;font-size:14px;color:#e8eaf0;line-height:1.65;">
            <strong style="color:#f0c040;">Quick note on pricing:</strong> Crystal Cruises fares are live today. We&#39;re actively adding Regent, Silversea, Atlas, and Explora — pricing will fill in steadily over the coming weeks.
          </td></tr>
        </table>
...
def send_email(service, to_email, name):
    html = build_html(name, next(r["opener"] for r in RECIPIENTS if r["name"] == name))
...
def main():
    svc = get_service()
    for r in RECIPIENTS:
        msg_id = send_email(svc, r["email"], r["name"])
