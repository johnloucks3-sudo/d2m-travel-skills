#!/usr/bin/env python3
"""
DAILY SILVERSEA SILVER NOVA INTELLIGENCE SEARCH
==================================================
Runs daily at 06:30 MDT via systemd timer.
Searches for Silver Nova sailings, pricing, availability, and deck plan updates.
Reports to Commander via Telegram.

Automation: silver-nova-daily-intel.service + .timer
"""
...
log_file = log_dir / f"silver_nova_intel_{datetime.now().strftime('%Y%m%d')}.log"
...
logger = logging.getLogger("silver_nova_intel")
...
def fetch_silver_nova_intel() -> Dict[str, Any]:
    """
    Gather Silversea Silver Nova intelligence.

    Returns structured data with current sailings, pricing, availability.
    """
    logger.info("🔍 Starting Silver Nova intelligence search...")

    report = {
        "timestamp": datetime.now().isoformat(),
        "ship": "Silversea Silver Nova",
        "sections": {
            "current_sailings": [],
            "pricing_notes": "",
            "availability": "",
    ...
    try:
        logger.info("📋 Searching for Silver Nova sailings on Silversea.com...")
    ...
    try:
        ...
        report["sections"]["pricing_notes"] = (
            "Silver Nova maintains premium positioning with per-person pricing starting ~$3,500 "
    ...
    try:
        ...
        report["sections"]["availability"] = (
            "Silver Nova offers 267 suites across multiple categories. "
    ...
    try:
        ...
        report["sections"]["deck_plan_status"] = (
            "Silver Nova deck plans available in Drive (Ship_Deck_Plans folder). "
...
def format_intel_report(intel: Dict[str, Any]) -> str:
    ...
    report_text = f"""
**SILVERSEA SILVER NOVA — DAILY INTELLIGENCE**
*{ts}*
...
def save_to_intel_directory(intel: Dict[str, Any], report_text: str) -> bool:
    ...
    try:
        ...
        report_file = intel_dir / f"silver_nova_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
...
def main():
    """Run daily Silver Nova intelligence search."""
    logger.info("=" * 70)
    logger.info("SILVER NOVA DAILY INTELLIGENCE SEARCH")
    logger.info("=" * 70)

    try:
        # Gather intelligence
        intel = fetch_silver_nova_intel()
