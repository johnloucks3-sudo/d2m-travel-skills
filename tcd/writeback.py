...
def _handle_auto_task(row: dict, decisions_path, overrides_path=None):
    ...
    if _is_delegated_work(row):
        ...
        # sourcePath uses TCD's "file:key" convention — e.g.
        # "OpsCenter/mission_board.json:missions", where ':missions' names the key
        # INSIDE the file, not part of the filename. Silver checks ground-truth
        # sources for filesystem existence, so passing the raw value made every
        # mission-board item fail with "named ground-truth source(s) do not exist"
        # — a valid source rejected on a naming convention.
        #
        # This silently held EVERY delegated item. Measured 2026-07-30 across the
        # 152-item stage-P backlog: 0 passed / 152 held with the fragment, and
        # 139 passed / 13 held once stripped — and those 13 are real holds
        # (genuinely vague criteria), which is the gate working as designed.
        # It is why the D and T stages had never been transited once.
        source = (row.get("sourcePath", "") or "").split(":", 1)[0]

        frame = silver_front_frame(
            row["id"], row.get("title", ""),
            acceptance_criteria=criteria,
            ground_truth_sources=[source] if source else [],
        )
        if not frame.ok:
