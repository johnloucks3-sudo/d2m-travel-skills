...
MODEL_TAGS = {
    ...
    "perplexity_reasoning": "Perplexity Reasoning Pro ($2/$8/1M)",
}
...
# DeepSeek V4 Pro: ~$0.305/M tokens, 1M context.
...
MODEL_MAP: Dict[TaskType, str] = {
    ...
    # OpenRouter FREE models only — NO PAID DEEPSEEK V3.1 (SO 2026-04-24 cost guardrail)
    TaskType.RESEARCH: "openrouter_free",         # lookups, cruise/flight/hotel/destination → Nemotron 120B free
    TaskType.OPERATIONAL: "openrouter_free",      # status checks, data lookups, routing → GPT-OSS 120B free
    TaskType.SUMMARIZATION: "openrouter_free",    # quick summaries → Gemma 3 27B free
    ...
    # Bulk context tiers — 1M context window, FREE OpenRouter (NOT paid DeepSeek V3.1)
    TaskType.CONTEXT_DUMP: "openrouter_free",     # DeepSeek R1 free tier ($0, SO 2026-04-24)
    TaskType.BULK_REVIEW: "openrouter_free",      # DeepSeek R1 free tier ($0, SO 2026-04-24)
    TaskType.SIMPLE_ANALYSIS: "gemini_lite",      # Gemini 2.5 Flash-Lite ($0.10/$0.40/1M)

    # Web-grounded research — Perplexity via OpenRouter (PAID — only if Commander approves)
...
def _call_anthropic(system_prompt: str, query: str,
    ...
    Fallback chain on auth/depleted/overload errors (SO 2026-04-24):
      Claude → OpenRouter FREE (Nemotron/GPT-OSS) → Groq (Llama 3.3) → Gemini Flash
      ⚠️ NEVER falls back to paid DeepSeek V3.1
    """
    import anthropic
    ...
    try:
        ...
        if any(code in err_str for code in ("401", "403", "529", "authentication", "api_key", "credit")):
            ...
            # Fallback 1: OpenRouter FREE models only (Nemotron 120B, NOT paid DeepSeek V3.1)
            if OPENROUTER_API_KEY:
                try:
                    return _call_openrouter(system_prompt, query,
                                           model=FREE_OPENROUTER_OPS,  # ← GPT-OSS 120B FREE
                                           max_tokens=max_tokens,
                                           temperature=temperature)
...
def route_and_call(system_prompt: str, user_prompt: str,
    ...
    # 3b. Handle OpenRouter FREE models — guardrail against paid DeepSeek V3.1 (SO 2026-04-24)

    # OpenRouter FREE tier — research, operational, summarization, bulk context dumps
    if model_id == "openrouter_free":
        ...
        logger.info("Auto-routing [%s] → %s (OpenRouter FREE, SO 2026-04-24)", task_type.value, model_name)
        try:
            response = _call_openrouter(system_prompt, user_prompt,
                                        model=selected_model,
                                        max_tokens=max_tokens,
                                        temperature=temperature)
            _log_model_usage(f"OpenRouter Free ({model_name})", persona_id, user_prompt,
                             tokens_est=(len(system_prompt + user_prompt) + len(response)) // 4,
                             task_type=task_type.value)
            return {
                "task_type": task_type.value,
                "model": selected_model,
                "model_tier": "openrouter_free",
                "engine": "openrouter",
                "response": response,
                "success": True,
            }
        except Exception as e:
            logger.warning("OpenRouter FREE failed (%s), falling back to Claude Haiku: %s", selected_model, e)
            model_id = CLAUDE_HAIKU
            tier = "haiku"
