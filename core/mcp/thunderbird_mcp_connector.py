...
def get_connector_router() -> APIRouter:
    ...
    async def mcp_sse_stream(request: Request):
        """
        MCP Streamable HTTP — SSE notification stream.

        Required by MCP 2025-03-26 spec: clients open a persistent GET
        connection to receive server-initiated notifications.  This server
        has no server-push events, so we send keepalive pings every 15 s
        to hold the connection open and satisfy Goose's transport layer.
        """
        ...
        if not session_id:
            session = connector.sessions.get_or_create()
            session_id = session.session_id

        async def _event_generator():
            # Send an initial endpoint event so the client knows the session ID
            yield (
                f"event: endpoint\n"
                f"data: {_json_mod.dumps({'session_id': session_id})}\n\n"
            )
            # Hold open with keepalive comments every 15 s
            while True:
