        ...
        try:
            def _structured_sitrep():
                ...
                response = client.messages.parse(
                    model="claude-haiku-4-5-20251001",  # Haiku — SITREP is classification/lookup, not creative (SO-2026-03-25)
                    max_tokens=1024,
                    system=(
...
async def cmd_persona(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Inject recent conversation context from persistent memory bridge
    recent_context = _bridge.get_recent_context(str(update.effective_chat.id), bot_source="d2mc2", limit=10)
