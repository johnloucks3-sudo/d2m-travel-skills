...
def get_a2a_router() -> APIRouter:
    ...
    # --- Goose-friendly send alias ---
    class SendTaskRequest(BaseModel):
        target_persona: str = Field("CLAUDE", description="Target: CLAUDE, COS, A2, A3, etc.")
        claude_role: Optional[str] = Field(None, description="Claude specialist role: CODER|ARCHITECT|STRATEGIST|ANALYST|DRAFTER|REVIEWER|TEACHER|ORACLE")
        content: str = Field(..., description="Task description / content")
        context_files: Optional[List[str]] = Field(None, description="Optional file paths for context")
        deliverable: Optional[str] = Field(None, description="Expected output type: working_code|spec_doc|analysis|draft|review|principle|recommendation")
        priority: str = Field("NORMAL", description="Task priority: HIGH|NORMAL|LOW")
        metadata: Optional[Dict[str, Any]] = None

    @router.post("/a2a/tasks/send", dependencies=[Depends(_verify_bearer)])
    async def send_task(req: SendTaskRequest):
        """Goose-friendly task dispatch alias for /a2a/tasks.

        Accepts a simplified payload and routes to the A2A task system.
        CLAUDE target_persona writes to claude_inbox.md for pickup.
        All other personas route through the standard persona executor.
        """
        pid = req.target_persona.upper()
        ...
        if req.claude_role:
            meta["claude_role"] = req.claude_role
        if req.deliverable:
            meta["deliverable"] = req.deliverable
        if req.context_files:
            meta["context_files"] = req.context_files
        meta["priority"] = req.priority

        # CLAUDE target: write to claude_inbox.md file relay
        if pid == "CLAUDE":
            ...
            claude_inbox = collab_dir / "claude_inbox.md"
            collab_dir.mkdir(parents=True, exist_ok=True)
            if not claude_inbox.exists():
                claude_inbox.write_text("# CLAUDE INBOX\n\n")
            ...
            role_tag = f"\nclaude_role: {req.claude_role}" if req.claude_role else ""
            ...
            entry = (
                ...
                f"priority: {req.priority}{role_tag}{deliv_tag}{ctx_tag}\n"
                f"pii: false\n"
                f"status: UNREAD\n"
                f"content: |\n  {req.content.strip()}\n"
            )
            with open(claude_inbox, "a") as f:
                f.write(entry)
            logger.info(f"A2A send → CLAUDE inbox: {task_id}")
            return {
                "task_id": task_id,
                "state": TaskState.SUBMITTED.value,
                "target": "CLAUDE",
                "channel": "claude_inbox.md",
        ...
        if pid == "GOOSE":
            ...
            deliv_tag = f"\ndeliverable: {req.deliverable}" if req.deliverable else ""
            ctx_tag = f"\ncontext_files: {', '.join(req.context_files)}" if req.context_files else ""
            from_tag = req.metadata.get("from", "CLAUDE") if req.metadata else "CLAUDE"
            subject_tag = f"\nsubject: {req.metadata.get('subject', '')}" if req.metadata and req.metadata.get("subject") else ""
            entry = (
                f"\n---\n## CLAUDE A2A TASK\n"
                f"task_id: {task_id}\n"
                f"msg_type: TASK\n"
                f"submitted_by: {from_tag}\n"
                f"authority: SPECIALIST\n"
                f"submitted_at: {now_str}\n"
                f"priority: {req.priority}{role_tag}{deliv_tag}{ctx_tag}{subject_tag}\n"
                f"status: UNREAD\n"
                f"content: |\n  {req.content.strip()}\n"
            )
        ...
        asyncio.create_task(_execute_task(task_id, pid, req.content))

        logger.info(f"A2A send task: {task_id} → {pid}")
        return {
            "task_id": task_id,
            "state": TaskState.SUBMITTED.value,
            "target": pid,
            "channel": "a2a_executor",
        }
