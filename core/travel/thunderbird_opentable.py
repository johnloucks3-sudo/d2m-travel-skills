...
# ============================================================================
# CORE LOGIC (plain functions — reusable outside the MCP tool wrapper)
# ============================================================================
...
async def search_restaurants(
    ...
    Returns a dict (not a JSON string) with up to 20 results: name, cuisine,
    price range, address, Michelin stars (if applicable), ratings, sample
    reviews, and a direct booking deep-link URL for each restaurant.

    Shared by the MCP tool wrapper and by non-MCP callers (e.g.
    core/dining/port_restaurant_coordinator.py) so the HTTP/parsing logic
    lives in exactly one place.
    """
    try:
        api_key, affiliate_id = _load_opentable_credentials()
    except ValueError as e:
        return json.loads(_stub_credentials_error("OpenTable", str(e)))
    ...
    try:
        ...
        return {"status": "error", "provider": "OpenTable", "message": str(e)}


def build_reservation_link_for(
    restaurant_id: str,
    date: str,
    time: str,
    party_size: int = 2,
) -> dict:
    """Generate a direct OpenTable reservation deep-link.

    No API call required — the link is constructed from the affiliate
    deep-link template. Shared by the MCP tool wrapper and non-MCP callers.
    """
    try:
...
def register_opentable_tools(mcp: FastMCP):
    ...
    async def search_opentable_restaurants(
        ...
        """
        result = await search_restaurants(city, cuisine, date, party_size)
    ...
    async def get_opentable_reservation_link(
        ...
        """
        result = build_reservation_link_for(restaurant_id, date, time, party_size)
        return json.dumps(result, indent=2)
