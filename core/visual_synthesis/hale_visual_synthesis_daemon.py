#!/usr/bin/env python3
"""
Hale Visual Synthesis Daemon Setup & Verification
===================================================

Manages the systemd service/timer for daily visual brief generation (05:30 MT).
...
Usage:
  python3 hale_visual_synthesis_daemon.py enable      # Enable daily generation
  python3 hale_visual_synthesis_daemon.py disable     # Disable daily generation
  python3 hale_visual_synthesis_daemon.py status      # Check timer status
  python3 hale_visual_synthesis_daemon.py trigger     # Run immediately (for testing)
  python3 hale_visual_synthesis_daemon.py logs        # View recent logs
  python3 hale_visual_synthesis_daemon.py verify      # Verify systemd setup
...
_SERVICE = "hale-visual-synthesis.service"
_TIMER = "hale-visual-synthesis.timer"
_LOGS = _ROOT / "logs"
...
def enable_timer():
    ...
    print(f"[{datetime.now(MT).strftime('%H:%M:%S')}] Enabling visual synthesis timer...")
...
def disable_timer():
    ...
    print(f"[{datetime.now(MT).strftime('%H:%M:%S')}] Disabling visual synthesis timer...")
...
def trigger_now():
    ...
    print(f"[{datetime.now(MT).strftime('%H:%M:%S')}] Triggering immediate visual synthesis...")
...
def verify_setup():
    ...
    checks = [
        ...
        ("Data generators exist", (_ROOT / "core" / "visual_synthesis" / "data_generators.py").exists()),
        ("Output directory exists", (_ROOT / "output" / "visuals").exists()),
    ]
...
Commands:
  enable      Enable daily generation at 05:30 MT
  disable     Disable daily generation
