...
| P1 | Watchdog dynamic failed-unit discovery | ⬜ | Hale | Kill arbitrary unit → watchdog SEES + restarts it |
| P2 | Off-box healthchecks heartbeat | 🔒 | Hale+Cmdr | Silence host → external Telegram page |
| P3a | Playwright re-login fallback (TESS/Centrav/Regent) | 🔒 | Sterling | 400 invalid_client → auto re-login |
| P3b | Scrape checkpoint/resume + TimeoutStartSec 300→900 | ⬜ | Sterling | Kill mid-scrape → resumes, no restart |
| P4a | OnFailure= Telegram on Tier-1 | ⬜ | Sterling | Fail a unit → immediate page |
| P4b | WatchdogSec + sd_notify on daemons | ⬜ | Sterling | Hang a daemon → systemd restarts |
| P4c | Fix misplaced StartLimit across units | ⬜ | Sterling | Audit clean |
| P5a | Differential fingerprint refresh | ⬜ | Sterling | Unchanged file skipped on re-run |
| P5b | Decision-based follow-up closure | ⬜ | Sterling | condition-triggered alert fires |
| P5c | Scoped subagent graceful degradation | ⬜ | Hale | Primary tool fails → documented fallback |
| X1 | Daily re-cert audit (3 thresholds) | ⬜ | Sterling | Audit runs daily, RED on breach |
