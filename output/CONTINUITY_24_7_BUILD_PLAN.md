...
| R8 | hale_brain_monitor manifest restore | ✅ | Sterling/Hale | Manifest present (hale_brain_manifest.md 10.9K, restored from archive); last run RESULT: SYNCHRONIZED; timer enabled 06:00; service exits clean. |
...
| C1 | TESS token re-auth | ✅ | Hale | SELF-HEALED autonomously via P3a Playwright re-login — fresh token acquired; tess-keepalive/-token-keepalive/-sync + boot-recovery cleared; TESS API returns live bookings. No Commander paste needed. Commit 768229bf |
...
| P2 | Off-box heartbeat | 🔨 staged | Hale+Cmdr | GitHub Actions external pinger (runners are off-box; Chromebook not needed) built + failure-injection tested BOTH paths (unreachable→alerts, alive→silent), YAML valid. NOT pushed — outward-facing activation held for Commander: deploy/offbox_heartbeat/ACTIVATE_P2.sh (2 cmds). Commits b0a3081a + fix |
| P3a | Playwright re-login fallback (TESS) | ✅ | Hale | CERTIFIED by live self-heal of a dead 400-invalid_client token: selector fix input[name=username] + JSON-safe vault writer + token-field map (ls.authenticationData.token) + JWT-exp expiry. 'Credential login OK 120min (CONFIRMED:TESS)'. Commit 768229bf |
...
| P5c | Scoped subagent graceful degradation | ✅ | Hale | core/ai_infra/agent_fallback.py with_fallback() injects machine-readable primary/fallback/total-failure block into dispatch prompts. Self-test PASS. Commit b0a3081a |
