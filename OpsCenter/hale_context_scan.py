"""
Hale Context Scan — DeepSeek Deep Read
========================================
Feeds all key Thunderbird documents to DeepSeek's massive context window.
...
# ── Token budget ──
# DeepSeek V3.1: ~128K context. Reserve 3K for output.
# Each char ~0.25 tokens (rough). Budget: ~125K tokens = ~500K chars.
...
def _call_deepseek(corpus: str) -> str:
    """Send corpus to DeepSeek and get back structured digest."""
...
Be specific. Use numbers where available. If something is unknown, say UNKNOWN, not guess."""
    ...
    try:
        ...
        return f"[CONTEXT SCAN ERROR] DeepSeek failed: {e}"
...
def run():
    ...
    print(f"[hale_context_scan] Sending to DeepSeek ({len(corpus):,} chars corpus)...")

    digest = _call_deepseek(corpus)

    header = f"# HALE — Session Context\n*Generated by DeepSeek deep scan at {ts}*\n*Documents read: {len(docs)} | Chars ingested: {total_chars:,}*\n\n---\n\n"
