    ...
    for msg_ref in all_msgs:
        ...
        try:
            ...
            task_prompt = (
                ...
                f"{safe_body}\n\n"
                f"REPLY POLICY — C2 RECALIBRATION 2026-07-29 (Commander directive: 'I get words back, no action').\n"
                f"An acknowledgement is NOT a deliverable. Never manufacture assurance.\n\n"
                f"You may reply ONLY in these two cases:\n"
                f"1. You can answer the substance RIGHT NOW from known facts → 'Roger —' + the direct answer. "
                f"An answer is substance; a promise to find the answer is not.\n"
                f"2. The action is ALREADY COMPLETE and you can cite a concrete artifact "
                f"(file path, mission id, thread id, sheet row, commit sha) → 'Done —' + what changed + the artifact.\n\n"
                f"In EVERY other case: DO NOT REPLY AT ALL. Exit without sending. The directive is captured to the "
                f"ledger and executed by core/comms/directive_executor.py, which sends a verified confirmation only "
                f"AFTER the work passes integrity_check.verify_and_record. Silence now is correct; a promise now is a lie.\n\n"
                f"BANNED — never emit these, they are the exact failure being fixed: 'Wilco', 'standing up', "
                f"'will confirm', 'report follows', 'once complete', 'logged to mission board for X to execute', "
                f"or any future-tense commitment.\n"
        ...
        for d_ref in d2mc_msgs:
            ...
            try:
                ...
                task_prompt2 = (
                    ...
                    f"2. The action is ALREADY COMPLETE and you can cite a concrete artifact "
                    f"(file path, mission id, thread id, sheet row, commit sha) → 'Done —' + what changed + the artifact.\n\n"
                    f"In EVERY other case — including every build/implementation task — DO NOT REPLY AT ALL. Exit without "
                    f"sending. Routing work to a queue is not an outcome and does not earn a reply. The directive is "
                    f"captured to the ledger and executed by core/comms/directive_executor.py, which sends a verified "
                    f"confirmation only AFTER the work passes integrity_check.verify_and_record.\n\n"
                    f"BANNED — never emit these, they are the exact failure being fixed: 'Wilco', 'standing up', "
                    f"'will confirm', 'report follows', 'Completion report follows', 'Sterling/ELON execute', "
                    f"'logged to mission board' as a standalone answer, or any future-tense commitment.\n"
                    f"Under 150 words. No formal header. No sign-off. No D2M branding."
                )
