# Innovation Scan — Weekly
**Generated:** 2026-05-31T01:30:05.913835
**Sources:** 24 scanned, 0 failed
**Findings:** 68 total

---

## Top 10 Findings (by engagement)

| # | Score | Source | Title | Category |
|---|-------|--------|-------|----------|
| 1 | 890 | github/trending | [LocoreMind/locoagent](https://github.com/LocoreMind/locoagent) | agents |
| 2 | 835 | github/trending | [simonlin1212/TradingAgents-astock](https://github.com/simonlin1212/TradingAgents-astock) | agents |
| 3 | 681 | hackernews | [SQLite is all you need for durable workflows](https://obeli.sk/blog/sqlite-is-all-you-need-for-durable-workflows/) | automation |
| 4 | 386 | hackernews | [MCP is dead?](https://www.quandri.io/engineering-blog/mcp-is-dead) | mcp |
| 5 | 290 | github/trending | [mikesheehan54/Claude-Code-Design-AI](https://github.com/mikesheehan54/Claude-Code-Design-AI) | agents |
| 6 | 252 | github/trending | [OpenOSINT/OpenOSINT](https://github.com/OpenOSINT/OpenOSINT) | mcp |
| 7 | 236 | github/trending | [Purewhiter/mobilegym](https://github.com/Purewhiter/mobilegym) | agents |
| 8 | 211 | github/trending | [capture0x/AdStrike](https://github.com/capture0x/AdStrike) | automation |
| 9 | 204 | github/trending | [JSingletonAI/dejavu](https://github.com/JSingletonAI/dejavu) | knowledge |
| 10 | 194 | github/trending | [yurineko73/Godot-MCP-Native](https://github.com/yurineko73/Godot-MCP-Native) | mcp |

## Agents (29 findings)

### [LocoreMind/locoagent](https://github.com/LocoreMind/locoagent)
- **Source:** github/trending | **Score:** 890
- **Relevance:** Direct | **Difficulty:** Medium
- AI-powered social media agent with real browser automation

### [simonlin1212/TradingAgents-astock](https://github.com/simonlin1212/TradingAgents-astock)
- **Source:** github/trending | **Score:** 835
- **Relevance:** Direct | **Difficulty:** Medium
- A股多Agent投研框架 — 适配A股数据源(龙虎榜/游资/解禁等)，7位分析师基于A股规则的辩论决策，基于TradingAgents深度改造，适配大A。A-share multi-agent investment research framework — 7 AI analysts, bull/bear debate, risk assessment。

### [mikesheehan54/Claude-Code-Design-AI](https://github.com/mikesheehan54/Claude-Code-Design-AI)
- **Source:** github/trending | **Score:** 290
- **Relevance:** Direct | **Difficulty:** Medium
- Claude Design: AI UI/UX architect. Screenshot to React, Figma components, Tailwind CSS generator. Prototyping agent, design systems, wireframe renderer. SVG icon creator, dark mode toggle, responsive 

### [Purewhiter/mobilegym](https://github.com/Purewhiter/mobilegym)
- **Source:** github/trending | **Score:** 236
- **Relevance:** Direct | **Difficulty:** Medium
- MobileGym: A Verifiable and Highly Parallel Simulation Platform for Mobile GUI Agent Research · 浏览器里运行的安卓模拟器 · Browser-hosted Android Simulator · Verifiable Evaluation · Scalable Online RL Training

### [aws-samples/sample-well-architected-skills-and-steering](https://github.com/aws-samples/sample-well-architected-skills-and-steering)
- **Source:** github/trending | **Score:** 142
- **Relevance:** Direct | **Difficulty:** Medium
- Reusable skills and steering that teach AI coding agents how to apply the AWS Well-Architected Framework. One set of playbooks, 12 supported tools.

### [modelstudioai/cli](https://github.com/modelstudioai/cli)
- **Source:** github/trending | **Score:** 137
- **Relevance:** Direct | **Difficulty:** Medium
- Official Model Studio CLI（阿里云百炼 CLI）built for AI Agent frameworks, exposing models, search, multimodal, and workflow capabilities as structured tool calls.

### [MemTensor/MemPrivacy](https://github.com/MemTensor/MemPrivacy)
- **Source:** github/trending | **Score:** 100
- **Relevance:** Direct | **Difficulty:** Medium
- MemPrivacy is a privacy-preserving personalized memory management framework for edge-cloud agents. 

### [CAPTCHAs can still detect AI agents](https://research.roundtable.ai/captchas-detect-ai/)
- **Source:** hackernews | **Score:** 82
- **Relevance:** Indirect | **Difficulty:** Medium

### [open-gitagent/langship.sh](https://github.com/open-gitagent/langship.sh)
- **Source:** github/trending | **Score:** 70
- **Relevance:** Direct | **Difficulty:** Medium
- The platform for shipping, governing AI agents — framework-agnostic, multi-runtime, GitOps-native, self-hosted.

### [ray-amjad/claude-code-workflow-creator](https://github.com/ray-amjad/claude-code-workflow-creator)
- **Source:** github/trending | **Score:** 70
- **Relevance:** Direct | **Difficulty:** Medium
- A Claude Code skill for authoring Workflow tool scripts: deterministic multi-agent orchestration. Preview of an unreleased feature.

### [madguyevans-creator/Fintech-CUI-Trust-Framework](https://github.com/madguyevans-creator/Fintech-CUI-Trust-Framework)
- **Source:** github/trending | **Score:** 65
- **Relevance:** Direct | **Difficulty:** Medium
- An open-source engineering governance standard defining trust boundaries for conversational AI agents in high-stakes domains. MIT Licensed.

### [adrianhajdin/react-native-lingua](https://github.com/adrianhajdin/react-native-lingua)
- **Source:** github/trending | **Score:** 62
- **Relevance:** Direct | **Difficulty:** Medium
- React Native Duolingo clone with a real-time AI voice teacher. Built with Expo, Stream Voice Agents, Clerk auth, and NativeWind for a complete, interactive mobile learning experience.

### [AndrewNgGirl/SkillLens](https://github.com/AndrewNgGirl/SkillLens)
- **Source:** github/trending | **Score:** 59
- **Relevance:** Direct | **Difficulty:** Medium
- Open-source self-hosted web tool for evaluating Agent Skills with rubric scores, Deep Review, and improvement suggestions.

### [Sendmux/website-feedback-widget](https://github.com/Sendmux/website-feedback-widget)
- **Source:** github/trending | **Score:** 58
- **Relevance:** Direct | **Difficulty:** Medium
- 📬 Drop-in feedback widget for websites & apps. Turn submissions into structured email & webhook data for AI agents. Framework-agnostic Web Component. Accessible, customisable, zero deps.

### [v2.1.154](https://github.com/anthropics/claude-code/releases/tag/v2.1.154)
- **Source:** blog/Claude Code Releases | **Score:** 50
- **Relevance:** Direct | **Difficulty:** Medium
- What's changed

Opus 4.8 is here! Now defaults to high effort · /effort xhigh for your hardest tasks
Introducing dynamic workflows: ask Claude to create a workflow and it orchestrates work across tens

## Mcp (20 findings)

### [MCP is dead?](https://www.quandri.io/engineering-blog/mcp-is-dead)
- **Source:** hackernews | **Score:** 386
- **Relevance:** Direct | **Difficulty:** Medium

### [OpenOSINT/OpenOSINT](https://github.com/OpenOSINT/OpenOSINT)
- **Source:** github/trending | **Score:** 252
- **Relevance:** Direct | **Difficulty:** Medium
- AI-powered OSINT agent with interactive REPL, MCP server, and CLI. 9 tools. Works with Claude, GPT-4, or local models. For authorized security research only.

### [yurineko73/Godot-MCP-Native](https://github.com/yurineko73/Godot-MCP-Native)
- **Source:** github/trending | **Score:** 194
- **Relevance:** Direct | **Difficulty:** Medium
- 一款支持godot开源引擎的的mcp插件，支持常见godot引擎操作，使用Godot原生实现MCPServer，无需安装依赖，开箱即用，支持多种AI工具调用。An MCP plugin that supports the open-source Godot engine, enabling common Godot engine operations. It implements the MCP 

### [Unagi-cq/cdp-bridge-mcp](https://github.com/Unagi-cq/cdp-bridge-mcp)
- **Source:** github/trending | **Score:** 185
- **Relevance:** Direct | **Difficulty:** Medium
- MCP server that bridges clients to a real browser through CDP and a companion extension.

### [testerlingcodo/gemini-antigravity-cli](https://github.com/testerlingcodo/gemini-antigravity-cli)
- **Source:** github/trending | **Score:** 171
- **Relevance:** Direct | **Difficulty:** Medium
- gemini antigravity 2.0 cli google terminal ai agent tool agy migration guide mcp server plugin slash commands gemini 3.5 flash coding agent tool free

### [Episkey-G/GrokSearch-rs](https://github.com/Episkey-G/GrokSearch-rs)
- **Source:** github/trending | **Score:** 164
- **Relevance:** Direct | **Difficulty:** Medium
- Rust MCP server for Grok web search and Tavily-backed source retrieval

### [wanghuan9/skill-manager](https://github.com/wanghuan9/skill-manager)
- **Source:** github/trending | **Score:** 159
- **Relevance:** Direct | **Difficulty:** Medium
- SkillDock is a skill manager and MCP manager for installing, viewing, updating, and syncing AI skills and MCP servers, with Git-aware updates to track upstream changes and local modifications. SkillDo

### [madguyevans-creator/resale-agent-skill-hub](https://github.com/madguyevans-creator/resale-agent-skill-hub)
- **Source:** github/trending | **Score:** 103
- **Relevance:** Direct | **Difficulty:** Medium
- AI-powered multi-platform C2C resale toolkit — 8 Claude Code skills + MCP server for automating second-hand selling

### [Aimino-Tech/opendocswork-mcp](https://github.com/Aimino-Tech/opendocswork-mcp)
- **Source:** github/trending | **Score:** 100
- **Relevance:** Direct | **Difficulty:** Medium
- Rust-native MCP server for Office document processing (Excel, Word, PowerPoint). Sub-millisecond, local-first, open source.

### [robbrad/homeassistant-mcp](https://github.com/robbrad/homeassistant-mcp)
- **Source:** github/trending | **Score:** 100
- **Relevance:** Direct | **Difficulty:** Medium
- Home Assistant MCP Server - Control your smart home with AI assistants via the Model Context Protocol

### [rahilp/second-brain-cloudflare](https://github.com/rahilp/second-brain-cloudflare)
- **Source:** github/trending | **Score:** 98
- **Relevance:** Direct | **Difficulty:** Medium
- One memory layer, every AI tool. Store anything once — recall it in Claude, ChatGPT, Cursor, or any MCP client. Self-hosted on Cloudflare's free tier.

### [mdowis/anansi](https://github.com/mdowis/anansi)
- **Source:** github/trending | **Score:** 90
- **Relevance:** Direct | **Difficulty:** Medium
- A self-healing web scraper built for hostile sites: selectors repair themselves, browser rendering kicks in when needed, and Chrome TLS fingerprinting evades bot detection. Ships with an MCP server so

### [atomicstrata/atomicmemory](https://github.com/atomicstrata/atomicmemory)
- **Source:** github/trending | **Score:** 77
- **Relevance:** Direct | **Difficulty:** Medium
- Portable semantic memory for AI agents: core engine, TypeScript SDK, framework adapters, MCP server, CLI, and host plugins.

### [v2.1.149](https://github.com/anthropics/claude-code/releases/tag/v2.1.149)
- **Source:** blog/Claude Code Releases | **Score:** 50
- **Relevance:** Direct | **Difficulty:** Medium
- What's changed

/usage now shows a per-category breakdown of what's driving your limits usage — skills, subagents, plugins, and per-MCP-server cost
/diff detail view can now be scrolled with the keybo

### [Release 2026.1.26](https://github.com/modelcontextprotocol/servers/releases/tag/2026.1.26)
- **Source:** blog/MCP Servers | **Score:** 50
- **Relevance:** Direct | **Difficulty:** Medium
- Release : v2026.1.26
Updated packages

@modelcontextprotocol/server-everything@2026.1.26
@modelcontextprotocol/server-memory@2026.1.26
mcp-server-time@2026.1.26


## Claude (10 findings)

### [Dynamic Workflows in Claude Code](https://claude.com/blog/introducing-dynamic-workflows-in-claude-code)
- **Source:** hackernews | **Score:** 187
- **Relevance:** Direct | **Difficulty:** Medium

### [imkunal007219/claude-coworker-model](https://github.com/imkunal007219/claude-coworker-model)
- **Source:** github/trending | **Score:** 127
- **Relevance:** Indirect | **Difficulty:** Medium
- Give Claude Code a cheap coworker. CLI tools that delegate bulk I/O to cheap LLMs (Kimi, DeepSeek, Ollama). Save 60-70% of your token budget.

### [longtundiary/ltd-ai-101](https://github.com/longtundiary/ltd-ai-101)
- **Source:** github/trending | **Score:** 94
- **Relevance:** Indirect | **Difficulty:** Medium
- LTD AI 101 — Claude Code course for absolute beginners (Thai). Build a stock research tool /brief from zero in 60-90 min.

### [FredAntB/Spec-Driven-Development](https://github.com/FredAntB/Spec-Driven-Development)
- **Source:** github/trending | **Score:** 75
- **Relevance:** Indirect | **Difficulty:** Medium
- A Claude skill that keeps your AI coding tools from contradicting  each other. Generates requirements.md, design.md, and tasks.md  before any code is written — then creates matching config files  for 

### [v2.1.153](https://github.com/anthropics/claude-code/releases/tag/v2.1.153)
- **Source:** blog/Claude Code Releases | **Score:** 50
- **Relevance:** Direct | **Difficulty:** Medium
- What's changed

Added skipLfs option to github/git plugin marketplace sources to skip Git LFS downloads during clone and update
Claude Code now shows a one-time notice when your npm global install can

### [Quoting Karen Kwok for Reuters Breakingviews](https://simonwillison.net/2026/May/31/anthropic-run-rate/#atom-everything)
- **Source:** blog/Simon Willison | **Score:** 50
- **Relevance:** Direct | **Difficulty:** Medium
- Anthropic defines “run-rate revenue” in two parts. Use the last 28 days of sales ⁠from customers charged on a consumption basis and multiply it by 13. Then, multiply the

### [How we contain Claude across products](https://simonwillison.net/2026/May/30/how-we-contain-claude/#atom-everything)
- **Source:** blog/Simon Willison | **Score:** 50
- **Relevance:** Direct | **Difficulty:** Medium
- How we contain Claude across products
A complaint I often have about sandboxing products is that they are rarely thoroughly documented, and in the absence of detailed documentation it's hard to

### [Anthropic's run-rate revenue hits $47 billion](https://simonwillison.net/2026/May/29/anthropic/#atom-everything)
- **Source:** blog/Simon Willison | **Score:** 50
- **Relevance:** Direct | **Difficulty:** Medium
- The most interesting thing about Anthropic's $65B Series H announcement is this line (emphasis mine):

Since our Series G in February, adoption has continued to grow across global enterprise customers

### [Claude Opus 4.8: "a modest but tangible improvement"](https://simonwillison.net/2026/May/28/claude-opus-4-8/#atom-everything)
- **Source:** blog/Simon Willison | **Score:** 50
- **Relevance:** Direct | **Difficulty:** Medium
- Anthropic shipped Claude Opus 4.8 today. My favourite thing about it is this note in the release announcement:

Users will find Opus 4.8 to be a modest but tangible improvement on its predecessor. The

### [llm-anthropic 0.25.1](https://simonwillison.net/2026/May/28/llm-anthropic/#atom-everything)
- **Source:** blog/Simon Willison | **Score:** 50
- **Relevance:** Direct | **Difficulty:** Medium
- Release: llm-anthropic 0.25.1


New model: Claude Opus 4.8 (claude-opus-4.8).
New -o 

## Automation (3 findings)

### [SQLite is all you need for durable workflows](https://obeli.sk/blog/sqlite-is-all-you-need-for-durable-workflows/)
- **Source:** hackernews | **Score:** 681
- **Relevance:** Indirect | **Difficulty:** Medium

### [capture0x/AdStrike](https://github.com/capture0x/AdStrike)
- **Source:** github/trending | **Score:** 211
- **Relevance:** Indirect | **Difficulty:** Medium
- AdStrike:AI Powered  modular Active Directory red-team framework for authorized penetration testing, AD enumeration,   attack-path analysis, Kerberos/ADCS workflows, reporting, and operator automation

### [Werner Herzog in conversation with Paul Cronin (2014)](https://fsgworkinprogress.com/2014/09/26/insignificant-bullets-evil-poachers-and-l-a-culture/)
- **Source:** hackernews | **Score:** 82
- **Relevance:** Indirect | **Difficulty:** Medium

## Knowledge (3 findings)

### [JSingletonAI/dejavu](https://github.com/JSingletonAI/dejavu)
- **Source:** github/trending | **Score:** 204
- **Relevance:** Indirect | **Difficulty:** Medium
- Memory that follows you across every AI tool. No cloud storage. No account required. Set it up once, use it everywhere.

### [Memory decline after menopause linked to loss of estrogen production in brain](https://news.northwestern.edu/stories/2026/05/memory-decline-after-menopause-linked-to-loss-of-estrogen-production-in-brain-tissue)
- **Source:** hackernews | **Score:** 147
- **Relevance:** Indirect | **Difficulty:** Medium

### [Release 2025.11.25](https://github.com/modelcontextprotocol/servers/releases/tag/2025.11.25)
- **Source:** blog/MCP Servers | **Score:** 50
- **Relevance:** Direct | **Difficulty:** Medium
- Release : v2025.11.25
Updated packages

@modelcontextprotocol/server-sequential-thinking@2025.11.25
@modelcontextprotocol/server-everything@2025.11.25
@modelcontextprotocol/server-filesystem@2025.11.2

## General (2 findings)

### [Corporate America Is Starting to Ration AI as Cost Skyrockets](https://www.wsj.com/tech/ai/corporate-america-is-starting-to-ration-ai-as-cost-skyrockets-1eb99d7a)
- **Source:** hackernews | **Score:** 163
- **Relevance:** Indirect | **Difficulty:** Medium

### [v2.1.152](https://github.com/anthropics/claude-code/releases/tag/v2.1.152)
- **Source:** blog/Claude Code Releases | **Score:** 50
- **Relevance:** Direct | **Difficulty:** Medium
- What's changed

/code-review --fix now applies review findings to your working tree after the review, surfacing reuse, simplification, and efficiency suggestions; /simplify now invokes /code-review --

## Devtools (1 findings)

### [SaroirCommunity/Spiderbrain-V3](https://github.com/SaroirCommunity/Spiderbrain-V3)
- **Source:** github/trending | **Score:** 62
- **Relevance:** Indirect | **Difficulty:** Medium
- SpiderBrain v3 is a multi-platform skill/framework to reduce token usage and AI hallucinations across Claude, Cursor, and other AI tools.
